...
import widi.mod.world.gen.generators.WorldGenAluminiumTree;
...
public class BlockSaplings extends BlockBush implements IGrowable, IMetaName, IHasModel{	
    ...
    public BlockSaplings(String name) {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(VARIANT, EnumHandler.EnumTypeMaterial.COPPER).withProperty(STAGE, Integer.valueOf(0)));
	...
	public IBlockState getStateFromMeta(int meta) {
		...
		return this.getDefaultState().withProperty(VARIANT, EnumHandler.EnumTypeMaterial.byMetadata(meta & 1)).withProperty(STAGE, Integer.valueOf((meta & 8) >> 3));
	...
	public void registerModels() {
		...
		for(int i = 0; i < EnumHandler.EnumTypeMaterial.values().length; i++){
			...
			Main.proxy.registerVariantRender(Item.getItemFromBlock(this), i, "sapling_" + EnumHandler.EnumTypeMaterial.values()[i].getName(), "inventory");
	...
	public void generateTree(World world, Random rand, BlockPos pos, IBlockState state) {
		...
		switch((EnumHandler.EnumTypeMaterial)state.getValue(VARIANT))
